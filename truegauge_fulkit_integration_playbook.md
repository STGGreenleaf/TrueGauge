...
**TrueGauge: Core Entities**

| Entity | What It Is | Example |
|:--|:--|:--|
| `DayEntry` | Daily sales record | date: "2026-03-11", netSalesExTax: 2450.00, notes: "Busy lunch" |
| `ExpenseTransaction` | Categorized expense | vendorName: "Sysco", category: "COGS", amount: 850.00 |
| `Settings` | Business config & goals | monthlyFixedNut: 15500, targetCogsPct: 0.35 |
| `CashSnapshot` | Point-in-time cash balance | amount: 45000, date: "2026-03-01" |

**Entity Chain:** `Settings` (goals/targets) → `DayEntry` (daily actuals) → `ExpenseTransaction` (costs) → **Health Calculations**

**CRUD:**
- **Create/Update/Delete:** DayEntry, ExpenseTransaction, CashSnapshot
- **Update only:** Settings
- **Read only:** Calculated metrics (health score, pace, survival %)

### 1.2 — What's the key metric?

Numbrly's key metric: **Margin %** = `(Price - Cost) / Price × 100`

**TrueGauge Key Metric: Health Score (0-100)**

A composite score combining pace, cost efficiency, and data confidence:

```
Health Score = paceScore (0-50) + cogsScore (0-20) + opexScore (0-15) + confidenceScore (0-15)

Where:
- paceScore = min(1.2, mtdNetSales / survivalGoal) / 1.2 × 50
- cogsScore = 20 - (overage × 100) if actualCogs > targetCogs, else 20
- opexScore = 15 - (overage × 30) if mtdOpex > expectedOpex, else 15  
- confidenceScore = (confidence / 100) × 15
```

**Supporting Metrics:**
- **Survival %** = `(mtdNetSales / survivalGoal) × 100`
- **Pace Delta** = `mtdSales - mtdTargetToDate` (hours-weighted)
- **Survival Goal** = `monthlyFixedNut / keepRate` where keepRate = `1 - COGS% - Fees%`

### 1.3 — What are the alerts?

Numbrly alerts on: low margin builds, builds below target, builds with no price set.

**TrueGauge Alert Conditions:**

- [x] **Behind pace** → `paceDelta < 0` (severity: warning)
- [x] **Below survival** → `survivalPercent < 100` (severity: warning if >80%, critical if ≤80%)
- [x] **COGS over target** → `actualCogs > targetCogsPct` (severity: warning)
- [x] **OPEX over budget** → `mtdOpex > expectedMonthlyOpex` (severity: warning)
- [x] **Low confidence** → `confidenceScore < 50` (severity: info)
- [x] **Cash below floor** → `cashBalance < operatingFloorCash` (severity: critical)
...
| "What if I hit 90% of my goal?" | `simulate_pace` |
| "Undo that" | `undo` |
